...
Montagens: 128ch e subconjunto 14ch ≈ nossa coroa (14 sítios; rótulo dizia 16ch até 06/09/2026) (mais próximo por posição 3D). Nulo: permutação.
...
# subconjunto 14ch ≈ nossa coroa (14 sítios; rótulo dizia 16ch até 06/09/2026): eletrodo Biosemi mais próximo (3D) de cada alvo 10-20
sub_idx = None
try:
    ...
    sub_idx = picks; print('14ch ≈ nossa (biosemi128 mais próximo):', [names[i] for i in sub_idx])
...
for F, tag in [(feats(X), '128 canais')] + ([(feats(X[:, sub_idx]), f'14 canais ≈ nossa coroa')] if sub_idx else []):
...
for F, tag in [(feats(X), '128 canais')] + ([(feats(X[:, sub_idx]), '14 canais ≈ nossa coroa')] if sub_idx else []):
