...
| Nieto 2022 (EEG, 128 wet ch, 4 Spanish words, 2 subjects × 3 sessions) | brain | **null**: 29.2 % (p = 0.03) and 25.4 % (p = 0.50) vs 25 % chance | **collapses across same-day sessions**: 27.5 % / 26.7 % ≈ chance | present in 1 of 2 subjects: 34 % with a 14-channel subset (p < 0.01) |
...
- It shows that imagined-speech decodability in EEG is **session-bound** in Nieto: present within a session for one subject with good wet electrodes and ~200 trials per condition, gone across sessions. Since the three sessions were recorded consecutively on the same day (see Method), this is a within-day drift/state effect with the cap presumably kept on, which is a harder finding for the field than a between-days effect would be, but it is *not* evidence about cap re-placement or day-to-day drift.
